...
function trx_demo_init() {
  ...
  $demo_panels
    .find('.trx_demo_tabs a:not([data-type="link"])')
    .on("click", function (e) {
      var tab = jQuery(this),
      ...
      if (!new_panel.hasClass("trx_demo_images_init")) {
        new_panel
          ...
          .each(function () {
            ...
            $self.attr("style", $self.data("style")).removeAttr("data-style");
          });
      }
  ...
  $demo_panels
    .find(".trx_demo_button_close,.trx_demo_panels_mask")
    .on("click", function (e) {
      $demo_panels
        .removeClass("open")
        ...
        .end()
        .find(".trx_demo_tab_active")
        .removeClass("trx_demo_tab_active");
      jQuery("html").removeClass("trx_demo_panels_open");
      e.preventDefault();
      return false;
    });
  $demo_panels
    .find(".trx_demo_panel_list_item[data-body-class] a")
    .on("click", function (e) {
      jQuery("body").toggleClass(
        jQuery(this).parents(".trx_demo_panel_list_item").data("body-class")
      );
      e.preventDefault();
      return false;
    });
  $demo_panels.find(".trx_demo_subscribe_email").on("keypress", function (e) {
    jQuery(this).removeClass("trx_demo_field_error");
  });
  $demo_panels.find(".trx_demo_subscribe_button").on("click", function (e) {
    jQuery(this).parents("form").submit();
    e.preventDefault();
    return false;
  });
  $demo_panels.find(".trx_demo_subscribe form").on("submit", function (e) {
    ...
    if (url === "" || email === "" || email.length < 7 || !regexp.test(email)) {
      ...
      jQuery
        .post(url, { action: "trx_demo_subscribe", nonce: nonce, email: email })
        .done(function (response) {
          ...
          var result = form
            .siblings(".trx_demo_message_box")
            .removeClass("trx_demo_message_box_error")
            .removeClass("trx_demo_message_box_success");
          if (rez.error === "") {
            form.get(0).reset();
  ...
  $demo_panels.find(".trx_demo_panel_filters_form").on("submit", function () {
    return false;
  });
  $demo_panels.find(".trx_demo_panel_filters_search").on("keyup", function (e) {
    filter_items(jQuery(this).parents(".trx_demo_panel"), false);
  });
  $demo_panels
    .find(".trx_demo_panel_filters_list")
    .on("click", "a", function (e) {
      ...
      if (!multi_filters) {
        ...
        if (
          $item.data("filter") &&
          $item.data("filter").toLowerCase() == "all"
        ) {
  ...
  function filter_items(section, hide_all) {
    ...
    if (!multi_filters) {
      flt =
        section
          .find(".trx_demo_panel_filters_list_item.filter_active")
          .eq(0)
          .data("filter") || "";
      flt = flt.trim();
    } else {
    ...
    if (hide_all) {
      ...
      section
        .find("[data-filter-value]:visible")
        .stop()
        .fadeOut(filter_show_timer);
    }
    txt = txt.toLowerCase();
    flt = flt.toLowerCase();
    setTimeout(
      function () {
        ...
        section
          .find(
            (flt != "" ? "[data-filter-value]" : "[data-search-value]") +
              (hide_all ? ":hidden" : "")
          )
          .each(function () {
            var $self = jQuery(this),
            ...
            if (
              (txt === "" || search_val.toLowerCase().indexOf(txt) >= 0) &&
              (flt == "all" ||
                flt == "" ||
                filter_compare(flt, flt_val.trim().toLowerCase()))
            ) {
  ...
  var skin_name = jQuery("body")
    .attr("class")
    .match(/\bskin_\S+/);
  if (skin_name) {
    skin_name = skin_name.toString().substring(5).replace(/-/g, " ");
    $demo_panels
      .find(".trx_demo_panel_skins .trx_demo_panel_list_item")
      .each(function () {
        var $self = jQuery(this);
        if (
          $self
            .find(".trx_demo_panel_list_item_title > a")
            .text()
            .trim()
            .toLowerCase() == skin_name
        ) {
